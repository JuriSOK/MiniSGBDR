...
pub struct BufferManager<'a>{
    ...
    disk_manager:RefCell<&'a DiskManager<'a>>,
...
impl<'a> BufferManager<'a>{
    ...
    pub fn new(db_config:&'a DBConfig, disk_manager:RefCell<&'a DiskManager<'a>>, algo_remplacement:&'a String)->Self
    {
    ...
    pub fn get_disk_manager(&self) -> &RefCell<&'a DiskManager>{
            ...
            */
            ...
            if  self.liste_pages[page_a_changer].get_pin_count()==0{
                if self.liste_pages[page_a_changer].get_dirty()==true{
                    self.disk_manager.borrow_mut().write_page(&page_id, &mut self.liste_buffer[page_a_changer].borrow_mut());
                }
    ...
    pub fn flush_buffers(&mut self){
        for i in 0..self.nb_pages_vecteur{
            if self.liste_pages[i as usize].get_dirty()==true{
                self.disk_manager.borrow_mut().write_page(self.liste_pages[i as usize].get_page_id(),&mut self.liste_buffer[i as usize].borrow_mut());
