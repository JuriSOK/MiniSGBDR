...
pub struct SelectOperator {
    conditions: Vec<Condition>,  // Conditions de sélection
...
impl SelectOperator {
    ...
    pub fn new(conditions: Vec<Condition>, child_iterator: Box<dyn IRecordIterator>, col_info: Rc<Vec<ColInfo>>) -> Self {
        SelectOperator { conditions, child_iterator, col_info }
    }
    ...
    fn evaluate_conditions(&self, record: &Record) -> bool {
        for condition in &self.conditions {
            ...
            if !condition.evaluate(record, self.col_info.clone().to_vec()) {
...
mod tests {
    ...
    #[test]
    fn test_relation_scanner_reset() {
        let record1 = Record::new(vec!["1".to_string(), "John".to_string()]);
        let record2 = Record::new(vec!["2".to_string(), "Jane".to_string()]);
        let records = vec![record1.clone(), record2.clone()];
        let mut scanner = RelationScanner::new(records);

        scanner.get_next_record();  // Consommer un enregistrement
        scanner.reset();  // Réinitialiser l'itérateur

        // Test que l'itérateur recommence depuis le début
        let result = scanner.get_next_record();
        assert_eq!(result, Some(record1), "Après réinitialisation, le premier enregistrement devrait être retourné.");
    }
    ...
    fn test_select_operator() {
        ...
        let condition = Condition::new_column_constant("id".to_string(), "=".to_string(), "1".to_string());
        let select_operator = SelectOperator::new(vec![condition], scanner, col_info);

        // Test de la sélection : seul l'enregistrement 1 doit correspondre
        let mut select_iterator = select_operator;
        let result = select_iterator.get_next_record();
        assert_eq!(result, Some(record1), "Le premier enregistrement devrait être sélectionné.");

        // Test que l'enregistrement 2 est rejeté
        let result = select_iterator.get_next_record();
        assert_eq!(result, None, "Le deuxième enregistrement ne devrait pas être sélectionné.");
    }

    #[test]
    fn test_select_operator_multiple_conditions() {
        let record1 = Record::new(vec!["1".to_string(), "John".to_string()]);
        let record2 = Record::new(vec!["2".to_string(), "Jane".to_string()]);
        let records = vec![record1.clone(), record2.clone()];
        let col_info = Rc::new(vec![
        ...
        let scanner = Box::new(RelationScanner::new(records));
        let condition1 = Condition::new_column_constant("id".to_string(), "=".to_string(), "1".to_string());
        let condition2 = Condition::new_column_constant("name".to_string(), "=".to_string(), "Jane".to_string());
        let select_operator = SelectOperator::new(vec![condition1, condition2], scanner, col_info);

        let mut select_iterator = select_operator;
        // Aucune condition ne correspond, donc aucun enregistrement ne sera retourné
        let result = select_iterator.get_next_record();
        assert_eq!(result, None, "Aucun enregistrement ne correspond aux conditions.");
    }
    ...
    fn test_projection_operator() {
        ...
        let scanner = Box::new(RelationScanner::new(records));
        let projection_operator = ProjectionOperator::new(vec!["id".to_string()], scanner, col_info);

        let mut projection_iterator = projection_operator;

        // Vérifier que seule la colonne "id" est projetée
        let result = projection_iterator.get_next_record();
        assert_eq!(result.unwrap().get_tuple(), vec!["1".to_string()], "La projection devrait ne contenir que la colonne id.");

        let result = projection_iterator.get_next_record();
        assert_eq!(result.unwrap().get_tuple(), vec!["2".to_string()], "La projection devrait ne contenir que la colonne id.");
    }

    #[test]
    fn test_projection_operator_multiple_columns() {
        let record1 = Record::new(vec!["1".to_string(), "John".to_string()]);
        let record2 = Record::new(vec!["2".to_string(), "Jane".to_string()]);
        let records = vec![record1.clone(), record2.clone()];
        let col_info = Rc::new(vec![
        ...
        // Vérifier que les colonnes "id" et "name" sont projetées
        let result = projection_iterator.get_next_record();
        assert_eq!(result.unwrap().get_tuple(), vec!["1".to_string(), "John".to_string()], "Les colonnes projetées devraient inclure 'id' et 'name'.");

        let result = projection_iterator.get_next_record();
        assert_eq!(result.unwrap().get_tuple(), vec!["2".to_string(), "Jane".to_string()], "Les colonnes projetées devraient inclure 'id' et 'name'.");
    }
